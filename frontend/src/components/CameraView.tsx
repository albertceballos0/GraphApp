...
const CameraView = ({ handleLogOut, token  }) => {
  ...
  const formularioModeloRef = useRef(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
        if (
            ...
            !formularioModeloRef.current.contains(event.target))
        ) {
  ...
  useEffect(() => {
    ...
    //prepared implica enlazar con el servidor de python saber si ya esta disponible
    ...
    socket.current.on('message', async (data) => {
      ...
      if (message_type === 'answer') {
        ...
        socket.current.emit('confirm', { type: 'confirm', 'token': token});
      }
    });
    ...
    return (() =>{
      if(mediaStream){
        ...
        tracks.forEach(track => track.stop()); // Detener cada track del stream
      }
      if(pcRef.current){
        pcRef.current.close();
      }
  ...
  const handleOffer = async (name) => {
    ...
    if(mediaStream){   
      ...
      console.log(token);
      socket.current.emit('message', { type: 'offer', sdp: pcRef.current.localDescription.sdp, 'token': token , 'name': name});
    }
  };
  ...
  const handleSubmitCrearModelo = async (event) => {
